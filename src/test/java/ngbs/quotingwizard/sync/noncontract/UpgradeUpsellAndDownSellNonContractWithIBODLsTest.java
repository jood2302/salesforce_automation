...
@Tag("NGBS")
@Tag("SyncWithNGBS")
public class UpgradeUpsellAndDownSellNonContractWithIBODLsTest extends BaseTest {
    private final Steps steps;

    //  Test data
    private final Product phoneToAdd;
    private final Product dlUnlimited;
    private final Product globalRingExLatam;
    private final Product globalRingExDigitalLine;
    ...
    public UpgradeUpsellAndDownSellNonContractWithIBODLsTest() {
        var data = JsonUtils.readConfigurationResource(
                "data/ngbs/sync/RC_MVP_Monthly_NonContract_WithIBODLs_186656013.json",
                Dataset.class);
        ...
        globalRingExLatam = data.getProductByDataNameFromUpgradeData("LC_IBO_290");
        globalRingExDigitalLine = data.getProductByDataNameFromUpgradeData("LC_DLI_282");
        ...
        expectedSyncSteps = List.of(CONTRACT_SYNC_STEP, DISCOUNT_SYNC_STEP, MANUAL_DOWN_SELL_STEP, UPGRADE_STEP, ORDER_SYNC);
    ...
    public void setUpTest() {
        ...
        steps.ngbs.purchaseAdditionalLicensesInNGBS(getItemFromTestData(globalRingExLatam.dataName, globalRingExLatam.existingQuantity));
    ...
    @Test
    @TmsLink("CRM-20476")
    @DisplayName("CRM-20476 - Upgrade + Upsell + Downsell with optional Discounts for Existing || New Contract.")
    ...
    public void test() {
        ...
        step("2. Select another package with a contract and save changes", () -> {
        ...
        step("3. Open Add Products tab and add a new Phone to the cart", () ->
                steps.quoteWizard.addProductsOnProductsTab(phoneToAdd)
        );

        step("4. Open the Price tab, set quantity for Global RingEX LATAM = " + globalRingExLatam.quantity + ", " +
                "set quantity for DL Unlimited = " + dlUnlimited.quantity + ", " +
                "set quantity for the new phone = " + phoneToAdd.quantity + ", " +
                "add discount for DL Unlimited, assign all new devices to DL", () -> {
            cartPage.openTab();
            cartPage.setNewQuantityForQLItem(globalRingExLatam.name, globalRingExLatam.quantity);
        ...
        step("7. Press 'Process Order' button on the Opportunity record page and follow the sync process to the end", () -> {
            ...
            //  Global RingEX™ - LATAM can only be removed (downsell) via its parent Global RingEX™ DigitalLine license
            steps.ngbs.downsellLicensesInNGBS(globalRingExDigitalLine);
